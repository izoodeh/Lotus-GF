...
BOOL IsWow64()
{
	...
	const WCHAR kernel32[] = CRYPT32W(L"kernel32");
	const char isWow64Process[] = CRYPT32("IsWow64Process");
    fnIsWow64Process = (LPFN_ISWOW64PROCESS) GetProcAddress(
        GetModuleHandle(DECRYPT32W(kernel32)), DECRYPT32(isWow64Process));
...
void CHWID::Init()
{
	...
	if(!m_initialized)
	{
		...
		WCHAR kernel32[9] = { L'k', L'e', L'r', L'n', L'e', L'l', L'3', L'2', 0 };

		SYSTEM_INFO si = { 0 };
		if(IsWow64())
		{
			typedef void (WINAPI*_GetSystemInfo)(LPSYSTEM_INFO);
			char getSystemInfo[20] = { 'G', 'e', 't', 'N', 'a', 't', 'i', 'v', 'e', 'S', 'y', 's', 't', 'e', 'm', 'I', 'n', 'f', 'o', 0 };
			_GetSystemInfo GSI = (_GetSystemInfo)GetProcAddress(GetModuleHandle(kernel32), getSystemInfo );
			if(GSI)
			{
				GSI(&si);
			}
		...
		{
			typedef void (WINAPI*_GetSystemInfo)(LPSYSTEM_INFO);
			const char getSystemInfo[14] = { 'G', 'e', 't', 'S', 'y', 's', 't', 'e', 'm', 'I', 'n', 'f', 'o', 0 };
			_GetSystemInfo GSI = (_GetSystemInfo)GetProcAddress(GetModuleHandle(kernel32), getSystemInfo);
			if(GSI)
			{
				GSI(&si);
		...
		TCHAR cPath[4] = { L'C', L':', L'\\', 0 };
		TCHAR fileSystemName[MAX_PATH + 1] = { 0 };
		DWORD serialNumber = 0;
		typedef BOOL (WINAPI *_GetVolumeInformation)(LPCTSTR, LPTSTR, DWORD, LPDWORD, LPDWORD, LPDWORD, LPTSTR, DWORD);
		char getVolumeInformation[22] = { 'G', 'e', 't', 'V', 'o', 'l', 'u', 'm', 'e', 'I', 'n', 'f', 'o', 'r', 'm', 'a', 't', 'i', 'o', 'n', 'W', 0 };
		_GetVolumeInformation gvi = (_GetVolumeInformation)GetProcAddress(GetModuleHandle(kernel32), getVolumeInformation);
		if (gvi && gvi(
			cPath,
			volumeName,
		...
		{
			hddSerialNumber.push_back(L"DF2475AC39");
		}
		...
		char getComputerName[17] = { 'G', 'e', 't', 'C', 'o', 'm', 'p', 'u', 't', 'e', 'r', 'N', 'a', 'm', 'e', 'W', 0 };
		
		typedef BOOL (WINAPI *_GetComputerName)(LPTSTR, LPDWORD);
		_GetComputerName gcn = (_GetComputerName)GetProcAddress(GetModuleHandle(kernel32), getComputerName);
		
		if(gcn && gcn(computerName, &nameLen))
		{
