...
void CAutoStart::Init()
{
	...
	if(m_enabled)
	{
		g_Log.Add(CLog::Blue, "[%S] Feature is enabled - loaded [%d] instance(s).", GetName(), m_app.size());
	}
}
...
void CAutoStart::LoadINI()
{
	...
	INT32 count = GetPrivateProfileInt(section, _T("Count"), 0, g_ConfigFile);
...
void CAutoStart::StartApps()
{
	...
	for(list<AutoStartData>::iterator Iter = m_app.begin();Iter!=m_app.end(); Iter++)
	{
		...
		{
			g_Log.Add(CLog::Blue, "[%S] Start: [%S]", GetName(), cmdLine);
