...
CVotePage::CVotePage() : m_enabled(FALSE), m_rewardHwidCheck(FALSE), m_requiredPoints(50), m_rewardId(0), m_rewardCount(0), m_multiplyReward(FALSE), m_interval(300), m_basePoints(0), m_setBasePoints(TRUE), m_newPoints(0), m_checkTime(0)
{
...
void CVotePage::LoadINI(const TCHAR* section)
{
	...
	m_rewardHwidCheck = GetPrivateProfileInt(section, _T("RewardHwidCheck"), 0, file);
	TCHAR temp[8192];
			...
			if(m_newPoints >= 0)
			{
				...
				if(itemId > 0 && itemCount > 0)
				{
					try
					{
						...
						g_UserDb.GiveItemToAll(itemId, itemCount, m_rewardHwidCheck ? true : false);
					}catch(...)
					{
						EXCEPTION_LOG;
					}
...
void CVotePage::CheckVotePoints(LPVOID param)
{
	if(param)
	{
		try
		{
			...
			INT32 points = lpVotePage->GetVotePoints();
			lpVotePage->HandleNewPoints(points);
			lpVotePage->Unlock();
		}catch(...)
		{
			EXCEPTION_LOG;
